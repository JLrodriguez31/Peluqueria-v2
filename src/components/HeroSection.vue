<template>
  <section id="home" class="relative z-10 px-6 pb-40 pt-13.25">
    <div class="mx-auto max-w-7xl text-center">
      <h1
        class="animate-fade-rise mx-auto max-w-xs md:max-w-7xl font-display text-4xl leading-[0.95] tracking-[-2.46px] text-white sm:text-6xl lg:text-8xl"
      >
        <em class="sm:text-black text-white not-italic">Más</em>
        allá del estilo, creamos
        <em class="sm:text-black text-white not-italic">tu esencia.</em>
      </h1>

      <p
        class="animate-fade-rise-delay mx-auto mt-10 max-w-xl xl:max-w-2xl text-base text-white sm:text-lg"
      >
        Corte, color y estilo pensados para resaltar lo mejor de ti. Tu imagen también comunica.
      </p>
      <a
        href="#reach-us"
        class="animate-fade-rise-delay-2 mt-12 inline-flex rounded-full bg-white px-6 py-3 md:px-14 md:py-5 text-base text-black transition-transform duration-300 hover:scale-[1.03]"
      >
        Reserva una cita
      </a>
    </div>
  </section>
</template>
